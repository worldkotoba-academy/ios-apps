            ...
            VStack(spacing: 16) {
                ...
                NavigationLink { StudyView(level: level, round: round) } label: {
                    ModeCard(icon: "book.fill", title: "学習モード",
                             detail: "1問ずつ解いてすぐに解説・音声を確認", color: .exAccent)
                }
                .buttonStyle(.plain)
                NavigationLink { ExamView(level: level, round: round) } label: {
                    ModeCard(icon: "timer", title: "本番モード",
                             detail: "制限時間\(round.durationMin)分・通しで解答して採点", color: .exRed)
                }
                .buttonStyle(.plain)
                history
            }
    ...
    private var info: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(level.official)
                ...
                .fixedSize(horizontal: false, vertical: true)
            if round.hasWritten {
                Label("記述式は解答例と評価ポイントを見ながらの自己採点です",
                ...
                ForEach(recs.prefix(10)) { rec in
                    HStack {
                        ...
                        Text(rec.reached ? "到達" : "未達")
                            ...
                            .foregroundColor(rec.reached ? .green : .exRed)
                    }
                    .padding(.vertical, 2)
                }
            }

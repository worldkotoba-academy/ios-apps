...
private let letterMarks = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"]
private let lowerMarks = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l"]

/// 選択肢の記号。本試験の表記に合わせて Config で切り替える。
/// shared＝大問共通の選択肢（対応づけ・段落挿入）。MARK_STYLE_SHARED が空なら MARK_STYLE と同じ
/// （DELE の B1 以上は3択が a/b/c、対応づけが A〜L と、1回の中で記号が混在する）。
func mark(_ i: Int, shared: Bool = false) -> String {
    let style = shared && !MARK_STYLE_SHARED.isEmpty ? MARK_STYLE_SHARED : MARK_STYLE
    let list: [String]
    switch style {
...
func isTargetLanguage(_ s: String) -> Bool {
    ...
    for u in s.unicodeScalars {
        ...
        case "thai":
            // 仮名が混じる行は日本語とみなす。ローマ字だけの行（タイ検5級の発音表記）はタイ語の音声で読ませない
            if (0x3040...0x30FF).contains(u.value) || (0xFF66...0xFF9F).contains(u.value) { return false }
            if (0x0E00...0x0E7F).contains(u.value) { hit += 1 }
        case "han":
            // 仮名が混じる行は日本語とみなす
            if (0x3040...0x30FF).contains(u.value) || (0xFF66...0xFF9F).contains(u.value) { return false }

            ...
            VStack(alignment: .leading, spacing: 6) {
                ForEach(Array(section.sharedChoices.enumerated()), id: \.offset) { i, c in
                    HStack(alignment: .top, spacing: 8) {
                        Text(mark(i + 1, shared: true)).font(.footnote.weight(.bold))
    ...
    private func choiceButton(_ i: Int, _ choice: String) -> some View {
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Text(mark(i, shared: q.choices?.isEmpty ?? true)).font(.body.weight(.semibold))
    ...
    private var feedback: some View {
        VStack(alignment: .leading, spacing: 10) {
            if q.kind == .write {
                ...
                let ansText: String = {
                    switch q.kind {
                    case .choice: return mark(q.answer ?? 0, shared: q.choices?.isEmpty ?? true)
                    case .multi:  return q.answerSet.sorted().map { mark($0) }.joined()
